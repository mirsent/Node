...
`createElement` 到底会返回什么呢？其实不是一个是一个 *实际的* DOM 元素。
...
我们吧这样的节点描述为“虚拟节点（virtual node）”，也常简写它为“VNode"。
...
在渲染函数中用 JavaScript 的 `if` / `else` 和 `map` 来重写：
...
通过 `this.$scopedSlots` 访问作用于插槽，每个作用于插槽都是一个返回若干 VNode 的函数：
...
如果要用渲染函数向子组件中传递作用于插槽，可以利用 VNode 数据对象中的 `scopedSlots` 字段：
